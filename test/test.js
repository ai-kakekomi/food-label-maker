...
/* 熱量のAtwater計算（6.2×4 ＋ 24.5×9 ＋ 60.1×4 = 485.7 → 486） */
ok(Engine.熱量を計算('6.2', '24.5', '60.1') === 486, 'たんぱく質6.2・脂質24.5・炭水化物60.1で486kcalになる',
  String(Engine.熱量を計算('6.2', '24.5', '60.1')));
ok(Engine.熱量を計算(0, 0, 0) === 0, 'すべて0なら0kcalになる');
ok(Engine.熱量を計算('0.1', '0', '0.02') === 0, '0.5未満は四捨五入で0kcalになる',
  String(Engine.熱量を計算('0.1', '0', '0.02')));
ok(Engine.熱量を計算('0.1', '0', '0.025') === 1, '0.5ちょうどは四捨五入で1kcalになる',
  String(Engine.熱量を計算('0.1', '0', '0.025')));
ok(Engine.熱量を計算('', '24.5', '60.1') === null, 'たんぱく質が未入力なら計算しない');
ok(Engine.熱量を計算('6.2', '', '60.1') === null, '脂質が未入力なら計算しない');
ok(Engine.熱量を計算('6.2', '24.5', '') === null, '炭水化物が未入力なら計算しない');
ok(Engine.熱量を計算('あ', '24.5', '60.1') === null, '数値でない入力は計算しない');
